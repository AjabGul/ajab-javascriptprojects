...
// step 11.1: set difficulty by getting from local storage
...
// step 8: focus method on text input upoun page load so user will type
text.focus();

// step 9: start the countdown of the timer
const timeInterval = setInterval(() => {
    // decrement by 1 sec 
    time--;
    // update DOM timeElement
    timeElement.innerHTML =`${time}s`;
    // check time expiry  
    if (time === 0){
        // stop countdown or clearinterval at 0
        clearInterval(timeInterval);
        // step 9.1: stop the game by showing the end game container
        gameOver();
    }

}, 1000);

// step 9.2: function for game over
function gameOver(){
...
// function to generate random word form wordList
function generateRandomWord(){
    return wordList[Math.floor(Math.random() * wordList.length)]
}


// step 5: function to add the random word to DOM
...
// step 7: function to update the score 
...
// Event Listeners 
// step 6: event listener on text input to compare with random word
text.addEventListener('input', (e)=> {
    // get value from user input
    const typeText = e.target.value;
    // check if user input match random word
    if(typeText === randomWord){
        // display a new word
        addWordToDOM();
        // step 7.1: update score
        updateScore();
        // clear input filled
        e.target.value = '';
        // add more time to clock upoun typing match according to the difficulty
        if(difficulty === 'easy'){
            time += 5;
...
// step 10: eventListener on setting button 
settingsBtn.addEventListener('click', () => {
    settings.classList.toggle('hide');
});

// step 11: event listener for changing the difficulty settings
